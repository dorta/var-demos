...
class HandTracker():
    ...
    def detect_hand(self, img_norm, timings=None):
        ...
        if timings is not None:
            timings["tracker_palm_infer_ms"] = (ti1 - ti0) * 1000.0
        ...
        if timings is not None:
            timings["tracker_palm_post_ms"] = (tp1 - tp0) * 1000.0
    ...
    def __call__(self, img, timings=None):
        t0 = time.perf_counter()
        tp0 = time.perf_counter()
        img_pad, img_norm, pad = self.preprocess_img(img)
        tp1 = time.perf_counter()
        if timings is not None:
            timings["tracker_preprocess_ms"] = (tp1 - tp0) * 1000.0

        try:
            td0 = time.perf_counter()
            source, keypoints = self.detect_hand(img_norm, timings)
            td1 = time.perf_counter()
            if timings is not None:
                timings["tracker_palm_total_ms"] = (td1 - td0) * 1000.0
            if source is None:
                if timings is not None:
                    timings["tracker_total_ms"] = (time.perf_counter() - t0) * 1000.0
                    timings["tracker_hand_found"] = 0
                return None, None
            ...
            if timings is not None:
                timings["tracker_warp_ms"] = (tw1 - tw0) * 1000.0

            tj0 = time.perf_counter()
            joints = self.predict_joints(img_landmark)
            tj1 = time.perf_counter()
            if timings is not None:
                timings["tracker_joint_infer_ms"] = (tj1 - tj0) * 1000.0
            ...
            if timings is not None:
                timings["tracker_project_ms"] = (tr1 - tr0) * 1000.0
                timings["tracker_hand_found"] = 1
        except Exception:
            kp_orig, box_orig = None, None
            if timings is not None:
                timings["tracker_hand_found"] = 0
                timings["tracker_error"] = 1
